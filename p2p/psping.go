...
func (p *PSPing) PingReq(dstpeerid string) ([10]int64, error) {
	...
	for i := 0; i < 10; i++ {
		...
		pingobj := &chestnutpb.PSPing{Seqnum: int32(i), IsResp: false, TimeStamp: time.Now().UnixNano(), Payload: payload[:]}
	...
	for {
		...
		if err == nil {
			if pingreqmsg.ReceivedFrom != p.PeerId {
				var pspingreq chestnutpb.PSPing
				if err := proto.Unmarshal(pingreqmsg.Data, &pspingreq); err != nil {
					return err
				}
				pingobj := &chestnutpb.PSPing{Seqnum: pspingreq.Seqnum, IsResp: true, TimeStamp: pspingreq.TimeStamp, Payload: pspingreq.Payload}
				bytes, err := proto.Marshal(pingobj)
				if err != nil {
					p.Topic.Publish(p.ctx, bytes)
				} else {
